...
def datetime2posix(t):
    '''converts a datetime array to a posix array'''
    posix = []
    for i in range(len(t)):
        posix.append(t[i].timestamp())
    return posix
...
def fillPixels(im, N=1):
    ...
    pixel, than replace the center pixel with a mean value of the neighbours
    """
    for n in range(N):
